...
test "metabolism consumes resources and produces waste" {
    ...
    // Energy: 500 + (10 * energy_per_resource) - (maintenance_base + 12/maintenance_byte_denom)
    const expected_energy = 500 + (10 * config.energy_per_resource) - (config.maintenance_base + 12 / config.maintenance_byte_denom);
    try std.testing.expectEqual(expected_energy, org.energy);
}

test "metabolism kills organism when energy too low" {
    const config = Config{};
    var org = Organism.initTest(.{ 32, 32, 16, 16 }, 5);
